                    ...
                    .show(ui, |ui| {
                        for (index, (label, command)) in entries.iter().enumerate() {
                            let response = ui.add_sized(
                                ...
                                egui::Button::new(*label).fill(if index == self.selected {
                                    theme::PANEL_ALT
                                } else {
                                    theme::PANEL
                                }),
                            ...
                            if index == self.selected && (up || down) {
                                response.scroll_to_me(Some(egui::Align::Center));
                            }
                        }
                    });
                if enter && let Some((_, command)) = entries.get(self.selected) {
                    action = Some(command.clone());
                }
                if entries.is_empty() {
...
fn command_list(page_ready: bool, can_save: bool) -> Vec<(&'static str, Action)> {
    let mut commands = vec![
        ...
        (
            "Show this window's history",
            Action::Section(ShellPage::History),
        ),
        (
            "Appearance and engine capabilities",
            Action::Section(ShellPage::Settings),
        ),
    ];
    if page_ready {
    ...
    if can_save {
        commands.push(("Save workspace locally", Action::SaveWorkspace));
    }
    commands
}
...
mod tests {
    ...
    fn unavailable_actions_are_not_presented_as_working_commands() {
        ...
        assert!(!commands.iter().any(|(_, action)| matches!(
            action,
            Action::Bookmark | Action::Resume | Action::SaveWorkspace
        )));
