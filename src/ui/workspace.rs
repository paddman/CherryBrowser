...
pub fn quick_links(ui: &mut egui::Ui) -> Option<Action> {
    ...
    ui.horizontal_wrapped(|ui| {
        for (title, url) in [
            ...
            if ui
                .add(egui::Button::new(title).min_size(egui::vec2(110.0, 38.0)))
                .on_hover_text(url)
                .clicked()
            {
...
pub fn bookmarks(ui: &mut egui::Ui, state: &mut Workspace, full: bool) -> Option<Action> {
    ...
    theme::card().show(ui, |ui| {
        theme::section_title(
            ui,
            "Bookmarks",
            "Keep useful pages close. Opened inside Cherry's own engine.",
        );
        ui.add_space(12.0);
        if full {
            ui.add(
        ...
        for (index, item) in state.data.bookmarks.iter().enumerate() {
            ...
            ui.push_id(index, |ui| {
                if ui
                    .add_sized([ui.available_width(), 34.0], egui::Button::new(&item.title))
        ...
        if count == 0 {
            ui.label(if state.data.bookmarks.is_empty() {
                "No bookmarks yet. Add a link below or save a loaded page with Ctrl/Cmd+D."
            } else {
        ...
        if full {
            ui.separator();
            ui.add_space(10.0);
            ui.label(if state.editing_url.is_some() {
                "Edit bookmark"
            } else {
                "Add a bookmark"
            });
            ui.add(
                egui::TextEdit::singleline(&mut state.draft_title)
                    .hint_text("Title")
                    .desired_width(f32::INFINITY)
                    .char_limit(120),
            );
            ui.add(
                egui::TextEdit::singleline(&mut state.draft_url)
                    .hint_text("https://example.com")
                    .desired_width(f32::INFINITY)
                    .char_limit(4096),
            );
            ui.horizontal_wrapped(|ui| {
                if theme::primary_button(ui, "Save link") {
                    state.save_draft();
                }
...
pub fn history(ui: &mut egui::Ui, urls: &[String], filter: &mut String) -> Option<Action> {
    ...
    theme::card().show(ui, |ui| {
        theme::section_title(
            ui,
            "History",
            "Completed navigation in this window. Never saved to the workspace file.",
        );
        ui.add_space(12.0);
        ui.add(
            egui::TextEdit::singleline(filter)
                .hint_text("Filter URLs")
                .desired_width(f32::INFINITY)
                .char_limit(256),
        );
        ...
        for (index, url) in urls.iter().enumerate().rev() {
            ...
            matches += 1;
            ui.push_id(index, |ui| {
                if ui.add(egui::Button::new(url)).clicked() {
                    action = Some(Action::Open(url.clone()));
                }
            });
        }
        if matches == 0 {
            ui.label(if urls.is_empty() {
                "No completed navigation yet."
            } else {
                "No matching history."
            });
        }
        ...
        if ui
            .add_enabled(
                !urls.is_empty(),
                egui::Button::new("Clear this window's history"),
            )
            .clicked()
        {
